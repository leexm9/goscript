...
func TestDefineResolveBuiltins(t *testing.T) {
	...
	for _, table := range []*SymbolTable{global, firstLocal, secondLocal} {
		for _, sym := range expected {
			...
			if result.Name != sym.Name || result.Scope != sym.Scope {
				t.Errorf("expected %s to resolve to %+v, got=%+v", sym.Name, sym, result)
			}
		}
	}
...
func TestResolveFree(t *testing.T) {
	...
	for _, tt := range tests {
		...
		for i, freeSymbol := range tt.expectedFreeSymbols {
			tmpResult := tt.table.FreeSymbols[i]
			if tmpResult != freeSymbol {
				t.Errorf("wrong free Symbol. want=%+v, got=%+v", freeSymbol, tmpResult)
			}
		}
	}
}
...
func TestDefineAndResolveFunctionName(t *testing.T) {
	...
	if result != expected {
		t.Errorf("expected %s to resolve %+v, gpt=%+v", expected.Name, expected, result)
	}
}
